...
export const healthHandler = rest.get(`${STELLAR_TESTNET_URL}/health`, (_req, res, ctx) => {
  return res(
    ctx.status(200),
    ctx.json(mockHealthResponse)
  );
...
export const rateLimitHandler = rest.get(`${STELLAR_TESTNET_URL}/health`, (_req, res, ctx) => {
  return res(
    ctx.status(429),
    ctx.json(rateLimitResponse)
  );
...
export const serverErrorHandler = rest.get(`${STELLAR_TESTNET_URL}/health`, (_req, res, ctx) => {
  return res(
    ctx.status(500),
    ctx.json(serverErrorResponse)
  );
...
export const notFoundHandler = rest.get(`${STELLAR_TESTNET_URL}/accounts/:accountId`, (_req, res, ctx) => {
  return res(
    ctx.status(404),
    ctx.json({ error: 'Account not found' })
  );
